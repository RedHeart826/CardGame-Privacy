# Privacy Policy — Reza's Suite

**Last updated: June 4, 2026**

## Overview
Card Game ("the App") is a card game app featuring Pasoor and Hokm. Your privacy is important to us.

## Data Collection
The App does **not** collect, store, or transmit any personal data. No accounts are required to use the App.

## Data Storage
All game progress, settings, and statistics are stored **locally on your device** using standard iOS storage. No data is sent to any server.

## Analytics & Tracking
The App does **not** use any analytics, advertising SDKs, or tracking technologies.

## Third-Party Services
The App does **not** integrate with any third-party data services.

## Children's Privacy
The App does not knowingly collect any information from children.

## Contact
If you have questions about this privacy policy, contact us at:
**reza.radman@gmail.com**

## Changes
We may update this policy from time to time. Changes will be reflected on this page.
